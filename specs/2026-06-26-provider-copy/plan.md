...
2. 在同一文件增加 `nextCopiedProviderName(sourceName: string)`，从当前 `providers` 列表收集已占用名称，按 `原名 (1)`、`原名 (2)` 递增生成第一个未占用名称。

3. 在 `ProvidersView.vue` 引入 `listProviderEndpoints`、`upsertProviderEndpoint`、`invalidateProviderEndpoints`。

4. 增加 `copyProvider(p: ProviderView)`：
   - 清空页面级错误状态。
   - 构造 `{ ...p, id: 0, name: nextCopiedProviderName(p.name) }` 请求体。
   - 调用复制 mutation。
