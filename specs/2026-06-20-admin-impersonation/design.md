...
1. **服务端**：`auth` 中间件在解析出真实用户后，若真实用户是管理员且请求带有 `X-PicoTera-Impersonation-User-Id`，则将写入请求上下文的用户替换为被冒充用户。下游所有按 `user_id` 隔离的查询、`requireAdmin` 能力闸门、`me` 接口因此全部以被冒充用户为准。
...
真实身份永远由既有鉴权来源（single-user-mode / http-header，通常由反向代理注入）解析得出，服务端可信。冒充 Header 是不可信的客户端附加提示，**只有当真实用户已被鉴权为管理员时才生效**。因此即使非管理员伪造 Header 也无法越权——其真实身份仍是非管理员，会被直接拒绝。
...
新增方法 `ResolveWithImpersonation`，封装既有 `Resolve` 并叠加冒充逻辑：
...
`Resolve` 保持不变（继续解析真实身份）。被冒充用户不做 `disabled` 校验——管理员可借此排查被禁用用户的数据。
...
成功时 `WithUser` 写入的是被冒充用户，下游逻辑（数据隔离、`requireAdmin`、`handleGetMe`）无需改动即可生效。

### 作用范围

`auth.Middleware` 仅挂载在 `/api/picotera` 子路由（`mgmtRouter`）。网关 catch-all、`/api/unified`、静态资源都不经过它，因此冒充机制天然只对管理接口生效，网关不受影响——无需任何额外开关。
...
- 用户名显示 `impersonation.target.displayName`，并加一个 `Tag`（`variant="accent"`，文案“冒充中”）；
...
还原按钮可见性由 store 驱动（不依赖 `isAdmin`），因此即便冒充普通用户使 `isAdmin` 翻转为 `false`、管理员导航隐藏，该按钮仍常驻可见。

### 图标：`src/ui/icons/paths.ts`

新增 `mask`（tabler `mask`，威尼斯面具，冒充语义）图标路径并加入 `IconName` 与 `iconComponents`。“还原”复用既有 `arrow-left`。

## 不改动项

- 无数据库迁移、无 sqlc 改动（复用既有 `GetUserByID`）。
- 无 OpenAPI / contract 改动——冒充 Header 在 chi 中间件读取，不是 Huma 操作参数，无需重新生成 `openapi.yaml` 与 TS 类型。
- 不做冒充审计日志：被冒充用户对数据的写入自然归属该用户，符合“以该用户身份操作”的语义。
