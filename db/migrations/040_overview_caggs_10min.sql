...
CREATE MATERIALIZED VIEW request_overview_bucketed
...
SELECT
  ...
  project_id,
  user_id,
  COALESCE(NULLIF(model_cost_currency, ''), '') AS cost_currency,
  ...
  SUM(COALESCE(model_cost, 0))::numeric(20, 6) AS cost
FROM request
WHERE type = 1
GROUP BY bucket_at, api_key_id, model, upstream_model, provider_id, project_id, user_id, cost_currency
WITH NO DATA;

ALTER MATERIALIZED VIEW request_overview_bucketed
  SET (timescaledb.materialized_only = false);

SELECT add_continuous_aggregate_policy(
  'request_overview_bucketed',
...
SELECT remove_continuous_aggregate_policy('request_overview_bucketed', if_exists => true);
DROP MATERIALIZED VIEW IF EXISTS request_overview_bucketed;
